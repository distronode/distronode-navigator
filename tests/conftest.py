...
from distronode_navigator.configuration_subsystem import to_sample
from distronode_navigator.configuration_subsystem.definitions import SettingsFileType
from distronode_navigator.configuration_subsystem.navigator_configuration import APP_NAME
from distronode_navigator.configuration_subsystem.navigator_configuration import NavigatorConfiguration
from distronode_navigator.configuration_subsystem.utils import parse_distronode_verison
...
from distronode_navigator.image_manager.puller import ImagePuller
from distronode_navigator.utils.functions import find_settings_file
...
def pytest_configure(config: pytest.Config):
    ...
    allow = ("DISTRONODE_NAVIGATOR_UPDATE_TEST_FIXTURES",)
    for k in os.environ:
        if k in allow:
            continue
        if k.startswith("DISTRONODE_"):
    ...
    # look for distronode.cfg
    _log, errors, details = parse_distronode_verison()
    if details is None:
        err = "\n".join(error.message for error in errors)
        pytest.exit(f"Error parsing distronode version:\n{err}")
    config_file = details["config file"]
    if config_file != "None":
        pytest.exit(f"Please remove the distronode config file '{config_file}' before testing.")

    # look for distronode-navigator settings file
...
def pytest_unconfigure(config: pytest.Config):
    """Restore the environment variables that start with DISTRONODE_.
