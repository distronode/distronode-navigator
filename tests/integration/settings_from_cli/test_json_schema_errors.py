...
class Scenario(BaseScenario):
    ...
    command: tuple[str, ...] = ("distronode-navigator", "-m", "stdout")
    """The command to run"""
...
test_data = (
    Scenario(
        ...
        settings_file=TEST_FIXTURE_DIR / "distronode-navigator_broken.yml",
    ),
    Scenario(
        comment="Unrecognized key",
        messages=("'unknown' was unexpected",),
        settings_file=TEST_FIXTURE_DIR / "distronode-navigator_unknown_key.yml",
    ),
    Scenario(
        comment="Unrecognized app",
        messages=("'non_app' is not one of ['builder',",),
        settings_file=TEST_FIXTURE_DIR / "distronode-navigator_no_app.yml",
    ),
    Scenario(
        comment="EE enabled is not a bool",
        messages=("5 is not one of [True, False]",),
        settings_file=TEST_FIXTURE_DIR / "distronode-navigator_not_bool.yml",
    ),
)
...
def test(data: Scenario, subtests: Any, tmp_path: Path):
    ...
    env = {"DISTRONODE_NAVIGATOR_CONFIG": str(data.settings_file), "NO_COLOR": "true"}
