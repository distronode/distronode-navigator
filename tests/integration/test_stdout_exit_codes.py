...
fixture_test_data = (
    StdoutTest(
        ...
        present="usage: distronode-config",
        return_code=0,
    ),
    StdoutTest(
        action_name="config",
    ...
    StdoutTest(
        ...
        action_params=(("cmdline", ["--help"]),),
        present="usage: distronode-inventory",
        return_code=0,
    ),
    StdoutTest(
        action_name="inventory",
...
class StdoutCliTest(NamedTuple):
    ...
    distronode_stdout: str
    """Distronode produced stdout"""
    distronode_stderr: str
    """Distronode produced stderr"""
    subcommand: str
    ...
    def command(self) -> list[str]:
        ...
        :returns: The command
        """
        return ["distronode-navigator", self.subcommand] + self.params + ["--mode", self.mode]
...
StdoutCliTests = (
    ...
    StdoutCliTest(
        ...
        return_code=1,
        distronode_stdout="",
        distronode_stderr="could not be found",
        navigator_stdout="",
...
def test_run_through_cli(
    ...
    assert data.distronode_stdout in stdout
    if not exec_env and not pae:
        # Without an EE and PAE, distronode writes to ``stderr``
        assert data.distronode_stderr in stderr
    else:
        # Everything is routed through ``stdout``
        assert data.distronode_stderr in stdout
