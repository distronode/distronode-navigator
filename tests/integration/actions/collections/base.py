...
EXPECTED_COLLECTIONS = ["distronode.builtin", "company_name.coll_1", "company_name.coll_2"]
...
base_steps = (
    ...
    UiTestStep(
        user_input=":back",
        comment="Back to distronode-navigator collections browse window",
    ...
    UiTestStep(
        user_input="Enter",
        comment="distronode-navigator collections browse window",
        present=EXPECTED_COLLECTIONS,
    ),
    ...
    UiTestStep(
        user_input=":0",
        comment="Browse distronode.builtin plugins window",
        present=["yum_repository"],
    ),
    UiTestStep(
        user_input=":1",
        comment="Browse distronode.builtin.add_host module",
        present=["distronode.builtin.add_host"],
    ),
)
...
class BaseClass:
    ...
    def fixture_tmux_session(
        ...
        params: TmuxSessionKwargs = {
            "setup_commands": [
                f"cd {tmp_coll_dir}",
                f"export DISTRONODE_COLLECTIONS_PATH={tmp_coll_dir}",
                "export DISTRONODE_DEVEL_WARNING=False",
                "export DISTRONODE_DEPRECATION_WARNINGS=False",
            ],
    ...
    def test(
        ...
        fixtures_update_requested = (
            ...
            or os.environ.get("DISTRONODE_NAVIGATOR_UPDATE_TEST_FIXTURES") == "true"
        )
