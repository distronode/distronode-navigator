...
CLI = Command(
    precommand="cd /tmp &&",
    raw_append=(
        "--eev $DISTRONODE_COLLECTIONS_PATH/collections:/tmp/collections_to_volmount:Z"
        " --senv DISTRONODE_COLLECTIONS_PATH=/tmp/collections_to_volmount"
    ),
...
initial_steps = (
    UiTestStep(
        user_input=CLI,
        comment="distronode-navigator collections top window",
        present=["distronode.builtin", "company_name.coll_1", "company_name.coll_2"],
    ),
)
