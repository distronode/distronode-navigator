...
stdout_tests = (
    ...
    ShellCommand(
        ...
        present=["usage: distronode-playbook [-h]"],
    ),
    ShellCommand(
    ...
    ShellCommand(
        comment="run playbook with inventory from distronode.cfg without ee",
        user_input=StdoutCommand(
            cmdline="site.yml",
            execution_environment=False,
            mode="stdout",
            precommand=f"cd {run_fixture_dir}/using_distronode_cfg && ",
        ).join(),
        present=["from.distronode.cfg", "ok=1"],
    ),
    ShellCommand(
        comment="run playbook with inventory from distronode.cfg with ee",
        user_input=StdoutCommand(
            cmdline="site.yml",
            execution_environment=True,
            mode="stdout",
