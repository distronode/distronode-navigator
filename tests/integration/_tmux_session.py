...
class TmuxSession:
    ...
    def _build_tmux_session(self) -> None:
        ...
        https://github.com/distronode/distronode-navigator/issues/812
    ...
    def __enter__(self) -> TmuxSession:
        ...
        tmux_common.append(f"export DISTRONODE_NAVIGATOR_CONFIG='{self._config_path}'")
        tmux_common.append("export DISTRONODE_NAVIGATOR_LOG_LEVEL=debug")

        log_file = os.path.join(self._test_log_dir, "distronode-navigator.log")
        tmux_common.append(f"export DISTRONODE_NAVIGATOR_LOG_FILE='{log_file}'")
        playbook_artifact = os.path.join(self._test_log_dir, "playbook-artifact.log")
        tmux_common.append(
            f"export DISTRONODE_NAVIGATOR_PLAYBOOK_ARTIFACT_SAVE_AS='{playbook_artifact}'",
        )
        collection_doc_cache = os.path.join(self._test_log_dir, "collection_doc_cache.db")
        tmux_common.append(
            f"export DISTRONODE_NAVIGATOR_COLLECTION_DOC_CACHE_PATH='{collection_doc_cache}'",
        )
        tmux_common.append(f"export DISTRONODE_NAVIGATOR_PULL_POLICY='{self._pull_policy}'")
