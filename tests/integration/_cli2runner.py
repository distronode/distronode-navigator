...
class Cli2Runner:
    ...
    cli_entry = "distronode-navigator {0} {1} -m {2}"
    ...
    def test_config_interactive(
        ...
        mocked_runner = mocker.patch(
            target="distronode_navigator.runner.distronode_config.get_distronode_config",
            side_effect=RunnerTestError,
        )
        monkeypatch.setenv("DISTRONODE_NAVIGATOR_ALLOW_UI_TRACEBACK", "true")
    ...
    def test_config_stdout(
        ...
        """
        mocked_runner = mocker.patch(
            target="distronode_navigator.runner.command.run_command",
            side_effect=RunnerTestError,
        )
        monkeypatch.setenv("DISTRONODE_NAVIGATOR_ALLOW_UI_TRACEBACK", "true")
    ...
    def test_inventory_interactive(
        ...
        """
        mocked_runner = mocker.patch(
            target="distronode_navigator.runner.distronode_inventory.get_inventory",
            side_effect=RunnerTestError,
        )
        monkeypatch.setenv("DISTRONODE_NAVIGATOR_ALLOW_UI_TRACEBACK", "true")
    ...
    def test_run_interactive(
        ...
        """
        mocked_runner = mocker.patch(
            target="distronode_navigator.runner.command_async.run_command_async",
            side_effect=RunnerTestError,
        )
        monkeypatch.setenv("DISTRONODE_NAVIGATOR_ALLOW_UI_TRACEBACK", "true")
