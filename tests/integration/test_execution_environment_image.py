...
from distronode_navigator import cli
...
test_data = [
    ("defaults", "", "distronode-navigator_empty.yml", {"container_image": default_ee_image_name()}),
    (
        "set at command line",
        f"--execution-environment-image {small_image_name()}",
        "distronode-navigator_empty.yml",
        {"container_image": small_image_name()},
    ),
    (
        "set in config file",
        "",
        "distronode-navigator_set_ee_image.yml",
        {"container_image": small_image_name()},
    ),
    (
        "set command line and config file, command line wins",
        f"--execution-environment True --execution-environment-image {default_ee_image_name()}",
        "distronode-navigator_set_ee_image.yml",
...
class Test(Cli2Runner):
    ...
    def run_test(
        ...
        monkeypatch.setenv("DISTRONODE_NAVIGATOR_CONFIG", cfg_path)
        monkeypatch.setenv("DISTRONODE_NAVIGATOR_COLLECTION_DOC_CACHE_PATH", str(coll_cache_path))
