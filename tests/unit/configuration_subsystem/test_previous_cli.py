...
from distronode_navigator.configuration_subsystem.definitions import CliParameters
from distronode_navigator.configuration_subsystem.definitions import Constants as C
...
def test_apply_previous_cli_mixed(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("DISTRONODE_NAVIGATOR_PASS_ENVIRONMENT_VARIABLES", "ENV1,ENV2")
    _messages, exit_messages = configurator.configure()
    assert not exit_messages
    monkeypatch.undo()
    ...
    monkeypatch.setenv("DISTRONODE_NAVIGATOR_SET_ENVIRONMENT_VARIABLES", "ENV1=VAL1")
    _messages, exit_messages = configurator.configure()
    assert not exit_messages
    monkeypatch.undo()
