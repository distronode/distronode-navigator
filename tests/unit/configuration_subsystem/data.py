...
BASE_EXPECTED = d2t(
    {
        "distronode_runner_artifact_dir": "/tmp/test1",
        "distronode_runner_rotate_artifacts_count": 10,
        "distronode_runner_timeout": 300,
...
CLI_DATA_CONFIG = [
    ...
    (
        "config dump -c /tmp/distronode.cfg",
        {"app": "config", "cmdline": ["dump"], "config": "/tmp/distronode.cfg"},
    ),
]
CLI_DATA_DOC = [
...
ENV_VAR_DATA = [
    ("distronode_runner_artifact_dir", "/tmp/test1", "/tmp/test1"),
    ("distronode_runner_rotate_artifacts_count", "10", 10),
    ("distronode_runner_timeout", "300", 300),
    ("distronode_runner_write_job_events", "false", False),
    ("app", "config", "config"),
    ("cmdline", "--forks 15", ["--forks", "15"]),
    ("collection_doc_cache_path", "/tmp/cache.db", "/tmp/cache.db"),
    ("config", "/tmp/distronode.cfg", "/tmp/distronode.cfg"),
...
SETTINGS = [
    ("distronode-navigator_empty.yml", "empty"),
    ("distronode-navigator.yml", "full"),
]
