...
from distronode_navigator.initialization import parse_and_update
...
def test_cli():
    ...
    assert sources.pop("distronode-navigator.app") == C.USER_CLI.value
    assert sources.pop("distronode-navigator.logging.level") == C.USER_CLI.value
    assert sources.pop("distronode-navigator.logging.append") == C.USER_CLI.value
    for path, source in sources.items():
        assert source in [C.AUTO.value, C.DEFAULT_CFG.value, C.NOT_SET.value, C.NONE.value], (
            path,
            source,
...
def test_env(monkeypatch: pytest.MonkeyPatch):
    ...
    assert sources.pop("distronode-navigator.app") == C.ENVIRONMENT_VARIABLE.value
    assert sources.pop("distronode-navigator.logging.level") == C.ENVIRONMENT_VARIABLE.value
    assert sources.pop("distronode-navigator.logging.append") == C.ENVIRONMENT_VARIABLE.value
    for path, source in sources.items():
        assert source in [C.AUTO.value, C.DEFAULT_CFG.value, C.NOT_SET.value, C.NONE.value], (
            path,
            source,
