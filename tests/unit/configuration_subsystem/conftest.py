...
from distronode_navigator.configuration_subsystem.definitions import SettingsSchemaType
from distronode_navigator.configuration_subsystem.navigator_configuration import NavigatorConfiguration
from distronode_navigator.utils.functions import ExitMessage
from distronode_navigator.utils.functions import LogMessage
...
@pytest.fixture
def distronode_version(monkeypatch):
    """Path the distronode --version call to avoid the subprocess calls.
    ...
    def static_distronode_version(command: Command):
        if command.command == "distronode --version":
            ...
            command.stdout = "distronode [core 2.12.3]\nconfig file = None"
        else:
