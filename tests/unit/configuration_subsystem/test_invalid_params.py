...
def test_ee_false_no_distronode(
    monkeypatch: pytest.MonkeyPatch,
    generate_config: GenerateConfigCallable,
):
    """Ensure an error is created if EE is false and distronode not present.

    :param monkeypatch: Fixture for patching
    :param generate_config: Fixture to generate a config
    """

    def check_for_distronode(*_args, **_kwargs):
        """Return the path to the container engine.

        :param _args: args
        :param _kwargs: kwargs
        :returns: no distronode
        """
        return ([], [ExitMessage(message="no_distronode")])

    monkeypatch.setattr(
        "distronode_navigator.configuration_subsystem.navigator_post_processor.check_for_distronode",
        check_for_distronode,
    )
    response = generate_config(params=["--ee", "false"])
    assert "no_distronode" in [exit_msg.message for exit_msg in response.exit_messages]
...
def test_not_a_bool(
    ...
    """
    monkeypatch.setattr("shutil.which", which)
    response = generate_config(settings_file_name="distronode-navigator_not_bool.yml")
    exit_msg = "In 'distronode-navigator.execution-environment.enabled': 5 is not of type 'boolean'."
    assert exit_msg in [exit_msg.message for exit_msg in response.exit_messages]
...
def test_poor_choices(
    ...
    if entry.cli_parameters and entry.cli_parameters.action == "store_true":
        ...
        # distronode-navigator choice error
