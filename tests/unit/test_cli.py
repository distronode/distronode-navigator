...
from distronode_navigator.cli import NavigatorConfiguration
from distronode_navigator.cli import main
from distronode_navigator.cli import parse_and_update
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            ...
            "quay.io/distronode/creator-ee:v0.9.2",
...
def test_update_args_general(_mf1, monkeypatch, given, argname, expected):
    ...
    """
    monkeypatch.setenv("DISTRONODE_NAVIGATOR_CONFIG", f"{FIXTURES_DIR}/unit/cli/distronode-navigator.yml")
...
def test_editor_command_default(_mf1, monkeypatch):
    ...
    """
    monkeypatch.setenv(
        "DISTRONODE_NAVIGATOR_CONFIG",
        f"{FIXTURES_DIR}/unit/cli/distronode-navigator_empty.yml",
    )
    args = deepcopy(NavigatorConfiguration)
    args.internals.initializing = True
    args.application_version = "test"
...
tst_hint_data = [
    ...
    pytest.param(
        TstHint(
            ...
            expected="with '--lf ~/distronode-navigator.log'",
        ),
        id="5",
    ),
