...
class Scenario(BaseScenario):
    ...
    def args(self, log_file: Path) -> list[str]:
        ...
        """
        arg_list = ["distronode-navigator", "--la", "false", "--lf", str(log_file), "--ll", "debug"]
...
def test_tz_support(
    ...
    log_file = tmp_path / "distronode-navigator.log"
    args = data.args(log_file=log_file)
    monkeypatch.setattr("sys.argv", args)
    monkeypatch.setattr("distronode_navigator.cli.wrapper", return_none)
