"""Test the functions exposed in the :mod:`~distronode_navigator.utils.functions` subpackage."""
...
from distronode_navigator.utils import functions
...
def test_find_many_settings_home(monkeypatch) -> None:
    ...
    """
    paths = [
        os.path.join(os.path.expanduser("~"), ".distronode-navigator" + ext) for ext in EXTENSIONS
    ]
...
def test_find_many_settings_cwd(monkeypatch) -> None:
    ...
    :param monkeypatch: The monkeypatch fixture
    """
    paths = [os.path.join(os.getcwd(), "distronode-navigator" + ext) for ext in EXTENSIONS]
...
def test_find_many_settings_precedence(monkeypatch) -> None:
    ...
    :param monkeypatch: The monkeypatch fixture
    """
    expected = os.path.join(os.getcwd(), "distronode-navigator.yml")
    paths = [expected, os.path.join(os.path.expanduser("~"), ".distronode-navigator.json")]
...
def test_env_var_is_file_path(
    ...
    """
    env_var = "DISTRONODE_NAVIGATOR_CONFIG"
    if set_env:
