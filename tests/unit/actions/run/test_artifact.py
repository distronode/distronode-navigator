...
def test_artifact_path(
    ...
    mocked_write = mocker.patch(
        "distronode_navigator.actions.run.serialize_write_file",
        return_value=None,
    )
...
def test_artifact_contents(monkeypatch: pytest.MonkeyPatch, mocker: MockerFixture):
    ...
    assert settings_entries["distronode-navigator"]["app"] == "run"

    settings_sources = mocked_write.call_args[1]["content"]["settings_sources"]
    assert settings_sources["distronode-navigator.app"] == Constants.USER_CLI.value
