...
from distronode_navigator.actions.run import Action as action
...
test_data = [
    Scenario(
        ...
        execution_environment_image="quay.io/distronode/network-ee:latest",
...
def test_runner_args(mocker: MockerFixture, data: Scenario):
    ...
    args.entry("distronode_runner_artifact_dir").value.current = data.private_data_dir
    args.entry("distronode_runner_rotate_artifacts_count").value.current = data.rotate_artifacts
    args.entry("distronode_runner_timeout").value.current = data.timeout
    args.entry("distronode_runner_write_job_events").value.current = data.write_job_events

    TestRunnerException = Exception
    command_async = mocker.patch(
        "distronode_navigator.actions.run.CommandAsync",
