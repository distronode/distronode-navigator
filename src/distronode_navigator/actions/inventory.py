...
from distronode_navigator.runner import DistronodeInventory
from distronode_navigator.runner import Command
from distronode_navigator.steps import Step
from distronode_navigator.ui_framework import Color
...
from distronode_navigator.ui_framework import Decoration
...
def content_heading(obj: Any, screen_w: int) -> CursesLines | None:
    ...
    operating_system = obj.get("distronode_network_os", obj.get("distronode_platform", ""))
...
class Action(ActionBase):
    ...
    def __init__(self, args: ApplicationConfiguration):
        ...
        self._runner: Command | DistronodeInventory
    ...
    def _collect_inventory_details_interactive(
        ...
        self._runner = DistronodeInventory(**kwargs)
    ...
    def _collect_inventory_details_automated(
        ...
        :param kwargs: The arguments for the runner call
        :raises RuntimeError: When the ``distronode-inventory`` command cannot be found for mode stdout
        :returns: The output, errors and return code from runner
        """
        if self._args.execution_environment:
            distronode_inventory_path = "distronode-inventory"
        else:
            exec_path = shutil.which("distronode-inventory")
            if exec_path is None:
                msg = "'distronode-inventory' executable not found"
                self._logger.error(msg)
                raise RuntimeError(msg)

            distronode_inventory_path = exec_path
        ...
        self._runner = Command(executable_cmd=distronode_inventory_path, **kwargs)
