...
from distronode_navigator.action_defs import RunStdoutReturn
...
from distronode_navigator.runner import DistronodeConfig
from distronode_navigator.runner import Command
from distronode_navigator.steps import Step
from distronode_navigator.ui_framework import CursesLine
from distronode_navigator.ui_framework import CursesLinePart
from distronode_navigator.ui_framework import CursesLines
from distronode_navigator.ui_framework import Interaction
from distronode_navigator.ui_framework import nonblocking_notification
from distronode_navigator.ui_framework import warning_notification
from distronode_navigator.utils.serialize import Loader
...
class Action(ActionBase):
    ...
    def __init__(self, args: ApplicationConfiguration):
        ...
        self._runner: DistronodeConfig | Command
    ...
    def run(self, interaction: Interaction, app: AppPublic) -> Interaction | None:
        ...
        :returns: The pending :class:`~distronode_navigator.ui_framework.ui.Interaction` or
            :data:`None`
        """
        ...
        notification = nonblocking_notification(
            messages=[
                "Collecting the distronode configuration, this may take a minute...",
            ],
        )
    ...
    def _run_runner(self) -> tuple | None:
        ...
        :raises RuntimeError: When the distronode-config command cannot be found with execution
        ...
        if self._args.display_color is False:
            set_env_vars["DISTRONODE_NOCOLOR"] = "1"
        ...
        kwargs = {
            ...
            "private_data_dir": self._args.distronode_runner_artifact_dir,
            "rotate_artifacts": self._args.distronode_runner_rotate_artifacts_count,
            "timeout": self._args.distronode_runner_timeout,
        }
        ...
        if self._args.mode == "interactive":
            self._runner = DistronodeConfig(**kwargs)
            kwargs = {}
            if isinstance(self._args.config, str):
                kwargs["config_file"] = self._args.config
            list_output, list_output_err = self._runner.fetch_distronode_config("list", **kwargs)
            dump_output, dump_output_err = self._runner.fetch_distronode_config("dump", **kwargs)
            if list_output_err:
                msg = f"Error occurred while fetching distronode config (list): '{list_output_err}'"
                self._logger.error(msg)
            if dump_output_err:
                msg = f"Error occurred while fetching distronode config (dump): '{dump_output_err}'"
                self._logger.error(msg)
            ...
            if self._args.execution_environment:
                distronode_config_path = "distronode-config"
            else:
                exec_path = shutil.which("distronode-config")
                if exec_path is None:
                    msg = "'distronode-config' executable not found"
                    self._logger.error(msg)
                    raise RuntimeError(msg)
                distronode_config_path = exec_path
            ...
            self._runner = Command(executable_cmd=distronode_config_path, **kwargs)
    ...
    def _parse_and_merge(self, list_output, dump_output) -> None:
        ...
        for key, value in parsed.items():
            ...
            if value["source"] == "default":
                ...
                value["default"] = False
            value["current_config_file"] = str(self._args.internals.distronode_configuration.path)
