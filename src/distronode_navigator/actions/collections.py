...
from distronode_navigator.utils.functions import path_is_relative_to
from distronode_navigator.utils.functions import remove_dbl_un
from distronode_navigator.utils.key_value_store import KeyValueStore
from distronode_navigator.utils.print import print_to_stdout
...
class Action(ActionBase):
    ...
    def run_stdout(self) -> RunStdoutReturn:
        ...
        if not args_updated or not isinstance(
            ...
            msg = (
                ...
                "Please review the distronode-navigator log file for errors."
            )
    ...
    def _run_runner(self) -> None:
        ...
        set_environment_variable["DISTRONODE_NOCOLOR"] = "True"

        # We mount in the utils directory to /opt/distronode_navigator_utils
        ...
        # at the end.
        if self._args.execution_environment:
            ee_navigator_utils_mount = "/opt/distronode_navigator_utils"
        ...
        if self._args.execution_environment:
            ...
            container_volume_mounts = [
                ...
                f"{utils_lib}:/opt/distronode_navigator_utils",
            ]
    ...
    def _parse(self, output) -> None:
        ...
        if isinstance(volume_mounts, list):
            ...
            for mount in volume_mounts:
                ...
                # /x/distronode_collections/co:/x/distronode_collections/co
                if dest_path.parent.name == "distronode_collections":
                    continue
                # /x/distronode_collections/co/ns:/x/distronode_collections/co/ns
                if dest_path.parent.parent.name == "distronode_collections":
                    continue
                # /x:/x
                if dest_path.name != "distronode_collections":
                    dest_path /= "distronode_collections"
