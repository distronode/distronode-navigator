...
from distronode_navigator.configuration_subsystem import Constants as C
from distronode_navigator.configuration_subsystem.definitions import ApplicationConfiguration
from distronode_navigator.runner import DistronodeDoc
...
class Action(ActionBase):
    ...
    def __init__(self, args: ApplicationConfiguration):
        ...
        self._runner: Command | DistronodeDoc
    ...
    def _run_runner(self) -> dict | tuple[str, str, int] | None:
        ...
        :raises RuntimeError: When the distronode-doc command cannot be found with execution
        ...
        if self._args.mode == "interactive":
            ...
            self._runner = DistronodeDoc(**kwargs)
            ...
            if self._args.execution_environment:
                distronode_doc_path = "distronode-doc"
            else:
                exec_path = shutil.which("distronode-doc")
                if exec_path is None:
                    msg = "'distronode-doc' executable not found"
                    self._logger.error(msg)
                    raise RuntimeError(msg)
                distronode_doc_path = exec_path
            ...
            self._runner = Command(executable_cmd=distronode_doc_path, **kwargs)
        ...
        if out:
            if isinstance(out, dict):
                ...
                try:
                    ...
                    if self._args.mode == "interactive":
                        self._logger.info(
                            "Parsing of distronode-doc output failed for '%s'",
                            self._plugin_name,
                        )
