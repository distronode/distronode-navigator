"""Some utilities that are specific to distronode_navigator."""
...
def check_for_distronode() -> tuple[list[LogMessage], list[ExitMessage]]:
    """Check for the distronode-playbook command, runner will need it.

    :returns: Exit messages if not found, messages if found
    """
    messages: list[LogMessage] = []
    exit_messages: list[ExitMessage] = []
    distronode_location = shutil.which("distronode-playbook")
    if not distronode_location:
        msg_parts = [
            "The 'distronode-playbook' command could not be found or was not executable,",
            ...
            "to use an Distronode Execution Environment",
        ]
        exit_messages.append(ExitMessage(message="\n".join(msg_parts)))
        return messages, exit_messages
    message = f"distronode-playbook found at {distronode_location}"
...
def find_settings_file() -> tuple[list[LogMessage], list[ExitMessage], str | None]:
    ...
    Find the file at ./distronode-navigator.(.yml,.yaml,.json),
    or ~/.distronode-navigator.(.yml,.yaml,.json).
    ...
    potential_paths.append([os.path.expanduser("~"), ".distronode-navigator"])
    potential_paths.append([os.getcwd(), "distronode-navigator"])
...
def generate_cache_path(app_name: str) -> Path:
    ...
    :param app_name: Name of application - currently distronode_navigator
