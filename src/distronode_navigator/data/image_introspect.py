...
class DistronodeCollections(CmdParser):
    """Available distronode collections collector."""

    @property
    def commands(self):
        """Define the distronode-galaxy command to list distronode collections.
    ...
    def parse(command: Command):
        """Parse the output of the distronode-galaxy command.

        :param command: The result of running the command
        """
        if "invalid choice: 'list'" in command.stderr:
            command.details = "This command is not supported with distronode 2.9."
            return
        collections = {}
...
class DistronodeVersion(CmdParser):
    """Distronode version collector."""

    @property
    def commands(self) -> list[Command]:
        """Define the distronode command to get the version.

        :returns: The defined command
        """
        return [Command(id_="distronode_version", command="distronode --version", parse=self.parse)]
