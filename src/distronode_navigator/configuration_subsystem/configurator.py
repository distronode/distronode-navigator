...
from distronode_navigator.utils.functions import shlex_join
from distronode_navigator.utils.json_schema import validate
from distronode_navigator.utils.serialize import SafeLoader
from distronode_navigator.utils.serialize import yaml
from distronode_navigator.utils.version_migration.migrate import MigrationType
from distronode_navigator.utils.version_migration.migrate import run_all_migrations
...
from .utils import parse_distronode_cfg
...
class Configurator:
    ...
    def _apply_settings_file(self) -> None:
        ...
        with open(settings_filesystem_path, encoding="utf-8") as fh:
            try:
                ...
                if config is None:
                    # In the case of distronode-navigator settings --sample > distronode-navigator.yml
        ...
        if errors:
            ...
            hint = (
                ...
                " (https://distronode-navigator.readthedocs.io/en/latest/settings/"
                "#distronode-navigator-settings)"
            )
            self._exit_messages.append(ExitMessage(message=hint, prefix=ExitPrefix.HINT))
            hint = (
                "The schema used for validation can be seen with"
                " 'distronode-navigator settings --schema'"
            )
            self._exit_messages.append(ExitMessage(message=hint, prefix=ExitPrefix.HINT))
            hint = (
                "A sample settings file can be created with"
                " 'distronode-navigator settings --sample'"
            )
            self._exit_messages.append(ExitMessage(message=hint, prefix=ExitPrefix.HINT))
            return
    ...
    def _retrieve_distronode_cfg(self):
        """Retrieve the distronode.cfg file.

        EE support is needed early on here so the post processors
        can have access to the distronode.cfg file contents as a fallback to
        navigators settings sources. The value won't be set but it is needed to
        determine where the distronode.cfg file should be pulled from
        """
        ee_enabled = str(self._config.execution_environment).lower() == "true"
        parsed_distronode_cfg = parse_distronode_cfg(ee_enabled=ee_enabled)
        self._messages.extend(parsed_distronode_cfg.messages)
        self._exit_messages.extend(parsed_distronode_cfg.exit_messages)
        self._config.internals.distronode_configuration = parsed_distronode_cfg.config
