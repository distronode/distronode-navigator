...
from distronode_navigator.utils.dict_merge import in_place_list_replace
...
def to_effective(
    ...
    for entry in settings.entries:
        ...
        if not isinstance(entry.value.current, Constants):
            ...
            if path == "distronode-navigator.distronode.cmdline":
                # post-processed into a list
                current = shlex_join(entry.value.current)
            elif path == "distronode-navigator.execution-environment.volume-mounts":
                current = []
...
def to_sample(settings: ApplicationConfiguration) -> tuple[str, str]:
    ...
    no_comment = ["---", "distronode-navigator:", "logging:", "level: debug", "append: False"]
    commented_lines = []
