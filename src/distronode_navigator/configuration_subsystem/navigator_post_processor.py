...
"""Post processing of distronode-navigator configuration."""
...
from distronode_navigator.utils.definitions import ExitPrefix
...
from distronode_navigator.utils.functions import check_for_distronode
from distronode_navigator.utils.functions import check_playbook_type
from distronode_navigator.utils.functions import flatten_list
from distronode_navigator.utils.functions import oxfordcomma
from distronode_navigator.utils.functions import str2bool
from distronode_navigator.utils.functions import to_list
...
class NavigatorPostProcessor:
    ...
    @staticmethod
    @_post_processor
    def distronode_runner_artifact_dir(
        entry: SettingsEntry,
        config: ApplicationConfiguration,
    ) -> PostProcessorReturn:
        """Post process distronode_runner_artifact_dir path.
    ...
    @staticmethod
    @_post_processor
    def distronode_runner_rotate_artifacts_count(
        entry: SettingsEntry,
        config: ApplicationConfiguration,
    ) -> PostProcessorReturn:
        """Post process distronode_runner_rotate_artifacts_count.
    ...
    # Post process distronode_runner_write_job_events.
    distronode_runner_write_job_events = _true_or_false

    @staticmethod
    @_post_processor
    def distronode_runner_timeout(
        entry: SettingsEntry,
        config: ApplicationConfiguration,
    ) -> PostProcessorReturn:
        """Post process distronode_runner_timeout.
    ...
    def execution_environment(self, entry: SettingsEntry, config) -> PostProcessorReturn:
        ...
        if entry.value.current is True:
            ...
            # https://github.com/distronode/distronode-navigator/issues/610
            ...
            new_messages, new_exit_messages = check_for_distronode()
    ...
    def inventory(entry: SettingsEntry, config: ApplicationConfiguration) -> PostProcessorReturn:
        ...
        if entry.value.current is C.NOT_SET:
            # Try the distronode.cfg file
            distronode_cfg = config.internals.distronode_configuration.contents
            if isinstance(distronode_cfg, dict):
                distronode_cfg_entry = distronode_cfg.get("defaults", {}).get("inventory")
                if isinstance(distronode_cfg_entry, str):
                    entry.value.current = [abs_user_path(i) for i in distronode_cfg_entry.split(",")]
                    entry.value.source = C.DISTRONODE_CFG
        else:
            # Use the specified
        ...
        try:
            ...
            if entry.cli_parameters:
                exit_msg = (
                    ...
                    " ~/distronode-navigator.log' to place it in your home directory"
                )
                exit_messages.append(ExitMessage(message=exit_msg, prefix=ExitPrefix.HINT))
        return messages, exit_messages
