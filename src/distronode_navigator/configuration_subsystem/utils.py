...
from distronode_navigator.command_runner import CommandRunner
...
def distronode_verison_parser(command: Command):
    """Parse the output of the distronode command.
    ...
    for entry in command.stdout_lines[1:]:
        ...
        if len(parts) == 2:
            ...
            msg = "distronode --version entry: '{entry}' could not be parsed"
            messages.append(LogMessage(level=logging.ERROR, message=msg))

    for key, value in details.items():
        msg = f"distronode --version: '{key}:{value}'"
        messages.append(LogMessage(level=logging.ERROR, message=msg))
...
@dataclass
class DistronodeConfiguration:
    """Data structure for an distronode.cfg file."""
...
@dataclass
class ParseDistronodeCfgResponse:
    """Data structure for the response of parse_distronode_cfg."""

    messages: list[LogMessage]
    """Log messages"""
    exit_messages: list[ExitMessage]
    """Exit messages"""
    config: DistronodeConfiguration = field(default_factory=DistronodeConfiguration)
    """An distronode configuration"""


def parse_distronode_cfg(ee_enabled: bool) -> ParseDistronodeCfgResponse:
    """Find the distronode.cfg file and parse it.

    If running without an EE, use distronode to get it.
    ...
    :returns: The distronode.cfg contents
    """
    response = ParseDistronodeCfgResponse(messages=[], exit_messages=[])
    response.config.path = Constants.NONE
    response.config.text = Constants.NONE
    response.config.contents = Constants.NONE

    if ee_enabled:
        msg = "EE support enabled: using current working directory for 'distronode.cfg'"
        response.messages.append(LogMessage(level=logging.DEBUG, message=msg))
        cfg_path = Path.cwd() / "distronode.cfg"
        if not cfg_path.exists():
            msg = (
                "EE support enabled: no 'distronode.cfg' found in current working directory."
                f" Tried {cfg_path!s}"
            )
            response.messages.append(LogMessage(level=logging.INFO, message=msg))
            return response

        msg = "EE support enabled: found 'distronode.cfg' in current working directory."
        response.messages.append(LogMessage(level=logging.INFO, message=msg))

    else:
        msg = "EE support disabled: using 'distronode --version' for 'distronode.cfg'"
        response.messages.append(LogMessage(level=logging.DEBUG, message=msg))
        new_messages, new_exit_messages, version_details = parse_distronode_verison()
        ...
        if cfg_file_name is None:
            msg = "EE support disabled: no 'config file' in 'distronode --version'"
            response.messages.append(LogMessage(level=logging.DEBUG, message=msg))
            return response
        if cfg_file_name == "None":
            msg = "EE support disabled: 'distronode --version' reports no config file"
            response.messages.append(LogMessage(level=logging.INFO, message=msg))
            return response
    ...
    try:
        ...
        exit_msg = f"The distronode configuration file '{cfg_path!s}' could not be parsed."
...
def parse_distronode_verison() -> tuple[list[LogMessage], list[ExitMessage], dict[str, Any] | None]:
    """Parse the output of the distronode --version command.
    ...
    command = Command(
        identity="distronode_version",
        command="distronode --version",
        post_process=distronode_verison_parser,
    )

    CommandRunner.run_single_process(commands=[command])
    if command.return_code:
        msg = "'distronode --version' reported the following errors:"
    ...
    msg = f"distronode --version stdout: '{command.stdout}'"
